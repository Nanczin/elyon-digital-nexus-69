...
const MemberAreaPreviewContent: React.FC<MemberAreaPreviewContentProps> = ({ settings, onLogout, memberArea, modules }) => {
  ...
  return (
    ...
    >
      ...
      <header 
        className="flex items-center justify-between h-[72px] px-4 sm:px-8 py-4"
        style={{ 
      ...
      {/* Conteúdo Principal */}
      <div className="flex-1 px-4 sm:px-8 py-8 sm:py-16 text-center space-y-6">
        <h1 className="text-2xl sm:text-3xl font-bold" style={{ color: textColor }}>
          Olá, {userName}!
        </h1>
        <p className="text-base sm:text-lg" style={{ color: secondaryTextColor }}>
            ...
            modulesToDisplay.map((module) => (
              ...
              >
                <div className="relative aspect-video w-full bg-gray-200 h-32 sm:h-48">
                  ...
                  {module.isCompleted && (
                    <div 
                      className="absolute top-2 right-2 sm:top-4 sm:right-4 p-1 sm:p-2 rounded-full flex items-center justify-center"
                ...
                <CardContent className="p-4 sm:p-6 space-y-3 sm:space-y-4 flex flex-col h-[calc(100%-8rem)] sm:h-[calc(100%-12rem)]">
                  ...
                  <Button 
                    ...
                    asChild
                  >
                    <Link to={`/membros/${memberArea?.id}/modules/${module.id}`}> {/* Corrigido o link para usar memberArea.id */}
