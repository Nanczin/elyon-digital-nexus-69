...
import MemberAreaPreviewContent from '@/components/member-area/MemberAreaPreviewContent'; // Importar o novo componente
import { Tables } from '@/integrations/supabase/types'; // Importar Tables

interface PlatformSettings {
  id: string; // This will now be a UUID generated by DB, not fixed
  user_id: string | null;
  member_area_id: string; // Now required and unique per member area
...
const getDefaultSettings = (memberAreaId: string, userId: string): PlatformSettings => ({
...
const AdminDesign = ({ memberAreaId: propMemberAreaId }: { memberAreaId?: string }) => {
  ...
  const [settings, setSettings] = useState<PlatformSettings>(() => 
    getDefaultSettings(currentMemberAreaId || '', user?.id || '')
  );
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [logoFile, setLogoFile] = useState<File | null>(null);
  const [isLoggedInPreview, setIsLoggedInPreview] = useState(false); // Novo estado para o preview de login
  const [memberAreaDetails, setMemberAreaDetails] = useState<MemberArea | null>(null); // Detalhes da área de membros
  const [modules, setModules] = useState<Module[]>([]); // Módulos da área de membros
  ...
  const fetchSettings = async () => {
    ...
    if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
      ...
      setSettings(deepMerge(getDefaultSettings(currentMemberAreaId!, user?.id || ''), data as Partial<PlatformSettings>));
    } else {
      setSettings(getDefaultSettings(currentMemberAreaId!, user?.id || '')); // Use default if no settings found
    }
    setLoading(false);
  };
  ...
  const fetchModulesForPreview = async () => {
    const { data, error } = await supabase
      .from('modules')
      .select('*') // Fetch all module details needed for the card
      .eq('member_area_id', currentMemberAreaId)
      .eq('status', 'published') // Only show published modules
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      const payload = {
        user_id: user?.id,
        member_area_id: currentMemberAreaId, // Use the current member area ID
        logo_url: newLogoUrl,
      ...
      const { error } = await supabase
        .from('platform_settings')
        .upsert(payload, { onConflict: 'member_area_id' }); // Upsert based on member_area_id
  ...
  const handleRestoreDefault = () => {
    setSettings(getDefaultSettings(currentMemberAreaId!, user?.id || ''));
    setLogoFile(null);
  ...
  const currentDefaultSettings = getDefaultSettings(currentMemberAreaId, user?.id || '');
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={settings.global_font_family || 'Inter'}
              ...
              >
                ...
                >
                  ...
                  <Button 
                    ...
                    onClick={handleMockLogin} // Adicionado onClick para o mock login
                  >
                    Entrar
                  </Button>
                </div>
