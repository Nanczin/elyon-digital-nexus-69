...
const MemberAreaLogin = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (error && error.code !== 'PGRST116') {
        console.error('MEMBER_AREA_LOGIN_DEBUG: Error fetching platform settings:', error);
      } else if (data) {
        setSettings(deepMerge(getDefaultSettings(memberAreaId), { ...data, colors: data.colors as PlatformSettings['colors'] } as Partial<PlatformSettings>));
      } else {
  ...
  useEffect(() => {
    const checkAccessAndRedirect = async () => {
      console.log('MEMBER_AREA_LOGIN_DEBUG: checkAccessAndRedirect triggered. User:', user?.id, 'MemberAreaId:', memberAreaId);
      if (user && memberAreaId) {
        ...
        if (profileError) {
          console.error('MEMBER_AREA_LOGIN_DEBUG: Error checking user profile for member area access:', profileError);
        } else if (profile) {
          console.log('MEMBER_AREA_LOGIN_DEBUG: User profile found. Profile member_area_id:', profile.member_area_id, 'URL memberAreaId:', memberAreaId);
          if (profile.member_area_id === memberAreaId) {
            console.log('MEMBER_AREA_LOGIN_DEBUG: Profile member_area_id matches URL. Redirecting to dashboard.');
            navigate(`/membros/${memberAreaId}`);
          } else {
            console.log('MEMBER_AREA_LOGIN_DEBUG: Profile member_area_id does NOT match URL. Not redirecting.');
          }
        } else {
          console.log('MEMBER_AREA_LOGIN_DEBUG: No user profile found for user ID:', user.id);
        }
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    console.log('MEMBER_AREA_LOGIN_DEBUG: Attempting sign in for email:', email);
    ...
    if (!error) {
      ...
      setTimeout(() => {
        console.log('MEMBER_AREA_LOGIN_DEBUG: Sign in successful. Redirecting to dashboard after delay.');
        navigate(`/membros/${memberAreaId}`);
      }, 500); // Reduced delay from 1000ms to 500ms, can be adjusted
    } else {
      console.error('MEMBER_AREA_LOGIN_DEBUG: Sign in failed:', error.message);
    }
    setLoading(false);
  };
