...
const Settings = () => {
  ...
  useEffect(() => {
    console.log('SETTINGS_DEBUG: useEffect triggered. user:', user?.id, 'authLoading:', authLoading); // Removido isAdmin do log
    // Não esperar por authLoading aqui, o redirecionamento será tratado abaixo
    // In a real app, you might fetch user-specific settings here
    // For now, just resolve loading after auth is settled
    setLoadingPage(false);
    console.log('SETTINGS_DEBUG: Auth settled, setLoadingPage(false).');
  }, [user]); // Removido authLoading das dependências

  // Removido: if (authLoading) { ... }

  if (!user) {
    console.log('SETTINGS_DEBUG: Redirecting to login because no user.');
    return <Navigate to="/auth/login" replace />;
  }

  // REMOVIDO: if (!isAdmin) { ... }

  if (loadingPage) { // Show loading for settings only after auth is done
    return <div className="flex items-center justify-center min-h-screen">Carregando configurações...</div>;
  }
